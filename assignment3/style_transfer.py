...
from assignment3.cs231n.image_utils import load_image, preprocess_image, deprocess_image
...
def content_loss(content_weight, content_current, content_original):
    ...
    - scalar content loss
    """
    deviations = tf.reduce_sum(tf.square(content_current - content_original))
    content_loss = content_weight * deviations
    return content_loss
...
def content_loss_test(correct):
    content_layer = 3
    content_weight = 6e-2
    c_feats = model.extract_features(content_img_test)[content_layer]
...
def gram_matrix(features, normalize=True):
    ...
    features = tf.reshape(tf.transpose(features), (shape[-1], -1))
    result = tf.matmul(features, tf.transpose(features))
    if normalize:
        result = result / tf.cast(tf.reduce_prod(tf.shape(features)), tf.float32)
    return result
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        ...
        loss += style_weights[i] * deviation
    return loss
...
def tv_loss(img, tv_weight):
    ...
    loss = tf.reduce_sum(tf.square(img[:, 1:, :, :] - img[:, :-1, :, :])) + \
...
def tv_loss_test(correct):
    ...
    error = rel_error(correct, student_output)
    print("Total variation loss error is {:.3f}".format(error))
